...
export default async function DesignsPage() {
  ...
  const [briefs, designs] = await Promise.all([
    prisma.designBrief.findMany({ where: { brandId: brand.id }, orderBy: { createdAt: 'desc' }, take: 5 }),
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Tasarım & Mockup Fabrikası</h1>
        <p className="text-muted-foreground">
          AI brief → görsel üret (Fal) → mockup (yerel) → Vision skorla → onayla.
        </p>
      </div>
