...
export async function ProductsTab({ brandId }: { brandId: string }) {
  const [designs, products, health] = await Promise.all([
    prisma.design.findMany({
      where: { brandId, status: { in: ['READY', 'PUBLISHED'] } },
  ...
  const printifyPassive = await settings.get<boolean>(brandId, 'printify.passive', true);
  const missingPiCount = products.filter(p => p.intelligence?.status !== 'READY').length;

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="max-w-2xl text-sm text-muted-foreground">
          Tasarımdan otomatik ürün sayfasıyla yayınla, mağazadan içe aktar, yaşam döngüsünü ve
          mağaza sağlığını yönet.
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          {missingPiCount > 0 && (
            <form action={backfillProductIntelligence}>
              <Button type="submit" variant="outline" size="sm">
                🔁 PI Backfill ({missingPiCount} ürün)
              </Button>
            </form>
          )}
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {designs.length === 0 ? (
            <p className="text-sm text-muted-foreground">Hazır tasarım yok (Tasarım Direktörü&apos;nde üret).</p>
          ) : (
            designs.map((d) => (
              <form
                key={d.id}
