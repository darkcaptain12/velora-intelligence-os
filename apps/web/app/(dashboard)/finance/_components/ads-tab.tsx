...
export async function AdsTab({ brandId, currency }: { brandId: string; currency: string }) {
  const [campaigns, metrics, limits, spend, autoMode, level] = await Promise.all([
    adCampaigns.list(brandId),
    adMetrics.latest(brandId, 20),
    spendLimits.list(brandId),
    adMetrics.spendByPeriod(brandId),
    settings.get<boolean>(brandId, 'ads.autoMode', false),
    settings.get<number>(brandId, 'autonomy.level', 1),
  ]);
              ...
              return (
                <div key={l.id} className="rounded-lg border p-4">
                  ...
                  <div className="mt-1 text-lg font-semibold">
                    {spent.toFixed(2)} / {Number(l.amount).toFixed(2)} {currency}
                  </div>
                </div>
              );
            })}
