...
export default async function FinancePage() {
  ...
  const [snaps, opScore, products, report] = await Promise.all([
    finance.latest(brand.id, 365),
    operationScores.latest(brand.id),
    prisma.product.findMany({
      where: { brandId: brand.id, OR: [{ price: { not: null } }, { cost: { not: null } }] },
      orderBy: { createdAt: 'desc' },
      take: 15,
    }),
    prisma.aIReport.findFirst({ where: { brandId: brand.id, type: 'FINANCE' }, orderBy: { createdAt: 'desc' } }),
  ]);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Finans & İş Zekası</h1>
          <p className="text-muted-foreground">Ciro, kâr, ROAS/CPA, vergi tahmini ve operasyon skoru.</p>
        </div>
