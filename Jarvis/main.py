...
from velora_bridge import parse_velora_intent, call_velora, speak_result, requires_confirmation, push_state
...
class JarvisLive:
    ...
    def _route_command(self, text: str) -> bool:
        """
        Komutu önce VELORA'ya yönlendirmeyi dener.
        VELORA intent eşleşirse True döner ve komutu işler.
        UNKNOWN ise False döner (Gemini yoluna devam et).
        """
        intent = parse_velora_intent(text)
        if intent == 'UNKNOWN':
            return False

        if requires_confirmation(text):
            self.ui.write_log(
                "SYS: ⚠️ Kritik komut — UI'dan onaylayarak çalıştırın: "
                f"'{text}'"
            )
            return True

        def _run():
            print(f"[VELORA] 🚀 {intent} → '{text}'")
            self.ui.set_state("THINKING")
            self.ui.write_log(f"VELORA → {intent}")
            result = call_velora(text)
            status = "✅" if result.get("ok") else "⚠️"
            msg = result.get("message", "")
            print(f"[VELORA] {status} {msg}")
            self.ui.write_log(f"VELORA {status}: {msg}")
            # say çalarken WakeWordListener'ı sustur — akustik geri beslemeyi önler
            self.set_speaking(True)
            speak_result(msg, on_end=lambda: self.set_speaking(False))
    ...
    def _on_text_command(self, text: str):
        ...
        if self._route_command(text):
            return  # VELORA yönlendirdi — Gemini'ye gönderme
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        return types.LiveConnectConfig(
            # WakeWord-First modda Gemini AUDIO üretir (model gereksinimidir).
            # Mikrofon _listen_audio kaldırıldığından VELORA komutları için
            # Gemini hiç input almaz → output üretmez → turn_complete tetiklenmez
            # → _is_speaking race condition yok. UNKNOWN fallback komutlarında
            # Gemini text alır, AUDIO yanıt üretir → hoparlörden çıkar.
        ...
        while True:
            ...
            try:
                ...
                async with (
                    ...
                    # WakeWord-First mod: _listen_audio() kaldırıldı.
                    # Mikrofon yalnızca WakeWordListener'a ait.
                    # Gemini yalnızca text komutu alır (_on_text_command → UNKNOWN fallback).
                    # Gemini'nin sesli ÇIKIŞI (_receive_audio + _play_audio) hâlâ aktif.
...
def main():
    ...
    def runner():
        ...
        # ── WakeWordListener: VELORA önce, Gemini fallback ──────────────────
        try:
            from wake_word import WakeWordListener

            def on_wake(text: str):
                print(f"[Jarvis] 🎤 Wake-word tetiklendi: '{text}'")
                ui.write_log(f"🎤 Jarvis: {text}")
                push_state('AWAKE', text)  # Orb: uyandı
                if not jarvis._route_command(text):
                    # VELORA eşleşmedi — Gemini'ye yönlendir
                    jarvis._on_text_command.__func__(jarvis, text)  # type: ignore[attr-defined]

            wake_listener = WakeWordListener(
                on_wake=on_wake,
                ui=ui,
                is_speaking_fn=lambda: jarvis._is_speaking,
            )
            wake_listener.start()
            print("[JARVIS] 🔊 WakeWordListener başlatıldı (VELORA entegrasyonu aktif).")
        except Exception as e:
            print(f"[JARVIS] ⚠️ WakeWordListener başlatılamadı: {e}")
